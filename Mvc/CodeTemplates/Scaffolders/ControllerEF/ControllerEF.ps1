...
$templateName = "ControllerWithRepository"

# do we have a BaseViewModel?
$viewModelBasePath = "BaseViewModel"
$viewModelBasePath = Join-Path Models $viewModelBasePath
if ($Area) {
	$areaPath = Join-Path Areas $Area
	$viewModelBasePath = Join-Path $areaPath $viewModelBasePath
}
Write-Host "Ensuring that a BaseViewModel is present in $Project"
$baseViewModelPathWithExt = $viewModelBasePath + ".cs"
if (-not (Get-ProjectItem $baseViewModelPathWithExt -Project $Project)) {
	Write-Host "Scaffolding BaseViewModel..."

	Add-ProjectItemViaTemplate $viewModelBasePath -Template "ViewModelBase" -Model @{ 
					Namespace = $defaultNamespace + ".Models"; 
	} -SuccessMessage "Added BaseViewModel {0}" -TemplateFolders $TemplateFolders -Project $Project -Force:$overwriteController
}
...
Write-Host "Ensuring that a BaseController is present in $Project"
$baseControllerPathWithExt = $baseControllerPath + ".cs"
if (-not (Get-ProjectItem $baseControllerPathWithExt -Project $Project)) {
	
	Write-Host "Scaffolding BaseController..."

	Add-ProjectItemViaTemplate $baseControllerPath -Template BaseController -Model @{
		ControllerNamespace = $controllerNamespace; 
	} -SuccessMessage "Added BaseController {0}" -TemplateFolders $TemplateFolders -Project $Project -Force:$overwriteController
}
