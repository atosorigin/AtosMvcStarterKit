...
$foundDbContextType = Get-ProjectType $DbContextType -Project $OutputProjectName -AllowMultiple
if (!$foundDbContextType) {
	...
	} -SuccessMessage "Added database context '{0}'" -TemplateFolders $TemplateFolders -Project $OutputProjectName -CodeLanguage $CodeLanguage -Force:$Force

	$foundDbContextType = Get-ProjectType ($dbContextNamespace + "." + $DbContextType) -Project $OutputProjectName
...
if (!$isDbContext) {
	...
	} -SuccessMessage "Added '$propertyName' to database context '$($foundDbContextType.FullName)'" -TemplateFolders $TemplateFolders -Project $OutputProjectName -CodeLanguage $CodeLanguage
}
}
